...
terminal_mode(){
	...
	while read site;do
		...
		if [ "$days_left" -gt "$warning_days" ];then
			printf "${ok_color}| %-10s | %-10s | %-10s | %-5s %s\n${end_of_color}" \
			"$sitename" "$certificate_last_day" "$days_left" "Ok"

		elif [ "$days_left" -le "$warning_days" ] && [ "$days_left" -gt "$alert_days" ];then
			printf "${warning_color}| %-10s | %-10s | %-10s | %-5s %s\n${end_of_color}" \
			"$sitename" "$certificate_last_day" "$days_left" "Warning"

		elif [ "$days_left" -le "$alert_days" ] && [ "$days_left" -gt 0 ];then
			printf "${alert_color}| %-10s | %-10s | %-10s | %-5s %s\n${end_of_color}" \
			"$sitename" "$certificate_last_day" "$days_left" "Alert"

		elif [ "$days_left" -le 0 ];then
			printf "${expired_color}| %-10s | %-10s | %-10s | %-5s %s\n${end_of_color}" \
	...
	printf "\n %-10s" "STATUS LEGEND"
	printf "\n ${ok_color}%-8s${end_of_color} %-10s" "Ok" "- More than ${warning_days} days left until the certificate expires"
	printf "\n ${warning_color}%-8s${end_of_color} %-10s" "Warning" "- The certificate will expire in less than ${warning_days} days"
	printf "\n ${alert_color}%-8s${end_of_color} %-10s" "Alert" "- The certificate will expire in less than ${alert_days} days"
	printf "\n ${expired_color}%-8s${end_of_color} %-10s\n\n" "Expired" "- The certificate has already expired"
}
